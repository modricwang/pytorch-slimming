...
class Logger:
    def __init__(self, args, state):
        if not state:
            self.state = dict()
            self.state['epoch'] = 0
            self.state['best_color_acc'] = 0
            self.state['best_type_acc'] = 0
        else:
            self.state = state
    ...
    def record(self, epoch, train_summary=None, test_summary=None, model=None):
        ...
        if test_summary:
            top1 = test_summary['color_acc'] + test_summary['type_acc']
            state_top1 = self.state['best_color_acc'] + self.state['best_type_acc']

            if top1 > state_top1:
                self.state['best_color_acc'] = test_summary['color_acc']
                self.state['best_type_acc'] = test_summary['type_acc']
    ...
    def final_print(self):
        print("- Best:  Color %6.3f  Type %6.3f" % (
            self.state['best_color_acc'],
            self.state['best_type_acc']))
